...
import { X, Settings, ChevronLeft, ChevronRight, RotateCw, Download, Eye, EyeOff, Undo, ZoomIn, ZoomOut, RotateCcw, Menu, ChevronUp, ChevronDown } from 'lucide-react';
...
export const AdvancedCropEditor: React.FC<AdvancedCropEditorProps> = ({
  ...
  const [mobileActiveTab, setMobileActiveTab] = useState<'view' | 'properties' | 'export'>('view');
  ...
  const getResizeHandle = useCallback((canvasX: number, canvasY: number) => {
    ...
    const handleSize = (isMobile ? 20 : 12) * previewScale; // Larger handles on mobile
  ...
  const drawPreview = useCallback(() => {
    ...
    if (showUncropped) {
      ...
      if (imgAspect > 1) {
        baseWidth = Math.min(maxWidth, originalImage.width * (isMobile ? 0.8 : 0.5));
        baseHeight = baseWidth / imgAspect;
      } else {
        baseHeight = Math.min(maxHeight, originalImage.height * (isMobile ? 0.8 : 0.5));
    ...
    if (showUncropped) {
      ...
      const handleSize = (isMobile ? 16 : 10) * previewScale;
  ...
  const handleMove = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    if ('touches' in e && e.touches.length === 2) {
      ...
      if (lastTouchDistance > 0) {
        ...
        const newScale = Math.max(0.1, Math.min(5, previewScale * scale));
    ...
    return (
      <div className="fixed inset-0 bg-black z-50 flex flex-col">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                mobileSettingsOpen ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
              }`}
            >
        ...
        <div className="flex-1 bg-gray-800 relative overflow-hidden">
          ...
          <div className="absolute bottom-4 right-4 flex flex-col space-y-2 bg-black/70 rounded-lg p-2">
            <button
              onClick={zoomIn}
              className="w-10 h-10 bg-white/20 hover:bg-white/30 rounded text-white text-lg font-bold transition-colors flex items-center justify-center"
            >
            ...
            <button
              onClick={resetZoom}
              className="w-10 h-10 bg-white/20 hover:bg-white/30 rounded text-white text-xs font-bold transition-colors flex items-center justify-center"
            >
          ...
          <div className="absolute bottom-4 left-4 bg-black/70 rounded px-3 py-1 text-xs text-white">
            {showUncropped ? 'Full Image' : 'Crop Only'} • {Math.round(crop.width)} × {Math.round(crop.height)}
        ...
        {mobileSettingsOpen && (
          <div className="absolute inset-x-0 bottom-0 bg-gray-900 border-t border-gray-700 max-h-[60vh] flex flex-col">
            {/* Mobile Tab Navigation */}
            <div className="flex border-b border-gray-700">
              <button
                onClick={() => setMobileActiveTab('view')}
                className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${
                  mobileActiveTab === 'view'
              ...
              <button
                onClick={() => setMobileActiveTab('export')}
                className={`flex-1 py-3 px-4 text-sm font-medium transition-colors ${
                  mobileActiveTab === 'export'
                    ? 'bg-blue-600 text-white'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800'
                }`}
              >
