...
export const AdvancedCropEditor: React.FC<AdvancedCropEditorProps> = ({
  ...
  }, [showUncropped, canvasToImageScale, previewScale, previewOffset, baseCanvasSize, crop]);
  ...
  const cropToCanvasCoords = useCallback((cropX: number, cropY: number) => {
    if (showUncropped) {
      ...
      // In crop-only view, convert from crop coordinates to preview coordinates
      const previewX = ((cropX - crop.x) / crop.width) * baseCanvasSize.width;
      const previewY = ((cropY - crop.y) / crop.height) * baseCanvasSize.height;
      
      return { 
        x: previewX * previewScale + previewOffset.x, 
        y: previewY * previewScale + previewOffset.y 
      };
    }
  }, [showUncropped, canvasToImageScale, previewScale, previewOffset, baseCanvasSize, crop]);
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isResizing && resizeHandle) {
      ...
      if (showUncropped) {
        imageDeltaX = deltaX / (canvasToImageScale.x * previewScale);
        imageDeltaY = deltaY / (canvasToImageScale.y * previewScale);
      } else {
        imageDeltaX = (deltaX / previewScale) * (crop.width / baseCanvasSize.width);
        imageDeltaY = (deltaY / previewScale) * (crop.height / baseCanvasSize.height);
      }
      ...
      // Ensure minimum size and image bounds
      newCrop.width = Math.max(10, Math.min(newCrop.width, originalImage?.width || 1000));
      newCrop.height = Math.max(10, Math.min(newCrop.height, originalImage?.height || 1000));
      newCrop.x = Math.max(0, Math.min(newCrop.x, (originalImage?.width || 1000) - newCrop.width));
      newCrop.y = Math.max(0, Math.min(newCrop.y, (originalImage?.height || 1000) - newCrop.height));
      ...
      // Calculate new position with image bounds
