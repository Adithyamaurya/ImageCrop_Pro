...
export const AdvancedCropEditor: React.FC<AdvancedCropEditorProps> = ({
  ...
  const drawPreview = useCallback(() => {
    ...
    if (showUncropped) {
      // Draw the full image at reduced opacity (NO ROTATION)
      ctx.globalAlpha = 0.3;
      ctx.drawImage(
        originalImage,
        0,
      ...
      // Draw the crop area at full opacity (NO ROTATION)
      ctx.globalAlpha = 1.0;
      ...
      // Draw the cropped portion at full opacity (NO ROTATION)
      ctx.drawImage(
        originalImage,
        clampedImageX,
        clampedImageY,
      ...
      // Apply rotation if present for the cropped image content
      const rotation = crop.rotation || 0;
      if (rotation !== 0) {
        ctx.save();
