...
import { X, Settings, ChevronLeft, ChevronRight, Download, Eye, EyeOff, Undo, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export const AdvancedCropEditor: React.FC<AdvancedCropEditorProps> = ({
  ...
  const canvasToCropCoords = useCallback((canvasX: number, canvasY: number) => {
    if (showUncropped) {
      ...
      // Return image coordinates directly (no need to convert to canvas space)
  ...
  const cropToCanvasCoords = useCallback((cropX: number, cropY: number) => {
    if (showUncropped) {
      // Convert image coords to canvas coords and apply zoom/pan
  ...
  const drawPreview = useCallback(() => {
    ...
    if (showUncropped) {
      ...
      // Convert crop coordinates from image space
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col lg:flex-row">
        ...
        {/* Right Sidebar - Settings (Desktop) / Bottom Panel (Mobile) */}
        <div className="w-full lg:w-80 bg-gray-900 border-t lg:border-t-0 lg:border-l border-gray-700 flex flex-col max-h-80 lg:max-h-none">
