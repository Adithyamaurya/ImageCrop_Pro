...
export const ViewportAwareCropCanvas: React.FC<ViewportAwareCropCanvasProps> = ({
  ...
  const draw = useCallback(() => {
    ...
    allCrops.forEach(crop => {
      ...
      // Draw resize handles for selected crop (larger for mobile) - NO ROTATION HANDLE
      if (isSelected) {
        ...
        // ROTATION HANDLE REMOVED - No longer drawing the yellow rotation button
      }
  ...
  // ROTATION HANDLE FUNCTION REMOVED - No longer detecting rotation handle clicks
  const getRotationHandle = (x: number, y: number, crop: CropArea) => {
    // Always return null - rotation handle is disabled
    return null;
  };
  ...
  const handleStart = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    // ROTATION HANDLE CHECK REMOVED - No longer checking for rotation handle
  ...
  const handleMove = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
    ...
    // ROTATION HANDLING REMOVED - No longer handling rotation
    ...
    if (!('touches' in e)) {
      ...
      if (canvas) {
        const selectedCrop = cropAreas.find(crop => crop.id === selectedCropId);
        if (selectedCrop) {
          // ROTATION HANDLE CURSOR REMOVED
  ...
  const handleEnd = () => {
    ...
    setIsDragging(false);
    setResizing(null);
    setRotating(null); // This state is no longer used but keeping for compatibility
    setIsPanning(false);
    setIsTouching(false);
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-gray-800/90 rounded-lg p-4 md:p-8 text-center max-w-md mx-4">
            ...
            <p className="text-xs md:text-sm text-gray-400">
              ...
              • Use resize handles to adjust size<br/>
